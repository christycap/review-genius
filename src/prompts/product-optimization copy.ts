export const AMAZON_REVIEW_LIMIT = 70 as const;
/**
 ...
 */
export const PRODUCT_OPTIMIZATION_PROMPT_VERSION = 9 as const;
...
- Treat the aggregate rating and total count as population-level context, not proof that the listing copy is effective. The supplied qualitative sentiment summaries were produced from up to ${AMAZON_REVIEW_LIMIT} recency-sorted reviews, with deliberate 1–3-star coverage and helpful-vote weighting. The extracted corpus is not a representative rating distribution; never infer prevalence or percentages from its positive and negative themes.
